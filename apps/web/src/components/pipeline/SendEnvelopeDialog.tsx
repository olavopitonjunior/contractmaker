...
export function SendEnvelopeDialog({
  ...
  // Reset quando o dialog abre. Testemunhas NÃO são mais auto-injetadas: o
  // operador as escolhe pelo botão "Selecionar testemunhas" (WitnessPicker).
  useEffect(() => {
    if (!open) return;
  ...
  // Chaves (e-mail/CPF) já presentes — o picker desabilita testemunhas repetidas.
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Limite do PLANO da conta ClickSign — a mensagem vem do servidor, que
        // a monta a partir da recusa da própria ClickSign.
        if (res.status === 402) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        {step === "edit" ? (
          ...
          <ReviewStep rows={rows} orderEnabled={orderEnabled} />
        )}
