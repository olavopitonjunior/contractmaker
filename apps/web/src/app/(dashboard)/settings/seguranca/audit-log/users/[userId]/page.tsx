...
/**
 * 1f — visão consolidada da atividade de UM usuário dentro do tenant: sumário
 * 30d + timeline recente. Gated por AUDIT_VIEW + membership do alvo na org
 * (evita vazar atividade cross-org). Server component: as datas são formatadas
 * uma vez no servidor (sem hydration mismatch), fixadas em America/Sao_Paulo.
 */
export default async function UserAuditPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link
          href="/settings/seguranca/audit-log"
          ...
          <tbody>
            ...
            {recent.map((r) => (
              <tr key={r.id} className="border-t">
                <td className="whitespace-nowrap px-3 py-2">{fmt(r.createdAt)}</td>
