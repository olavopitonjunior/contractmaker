...
| Certidões | `lib/certidoes/*` (planner/executor/endpoints/outcome-classifier) | Feito em 02/09/2026: `planCertidoesForDeal(..., { esteira: "locacao" })` lê `locatarios`/`garantia.fiador` (+ cônjuge)/`locadores`/`imovel`; alvos `locatario`/`fiador`/`conjuge_fiador`/`locador`; feature `locacao.certidoes` (default OFF); aba no negócio de locação (PR seguinte) |
