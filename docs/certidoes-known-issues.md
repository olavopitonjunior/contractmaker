...
| Sintoma | Causa | Tratamento / Prevenção |
|---|---|---|
| **606** "O parâmetro 'birthdate' deve ser preenchido quando o campo 'CPF' for usado" (TRF5 etc.) | o handler do TRF individual em `planner.ts` montava o job PF só com `cpf`+`nome`, SEM `birthdate` (ao contrário de Receita/PGFN/TJSP) → o portal recusava mesmo com a data no formulário | **Fix 2026-06-05:** o handler anexa `birthdate: normalizeDate(parte.data_nascimento)` p/ PF. Validado: TRF5 do vendedor passou de 606 "faltam dados" → success+PDF. |
| **608** "Data de nascimento preenchida e … na receita federal **não coincidem**" (TRF5) | o TRF5 NÃO só usa a data — ele **cruza contra a Receita** (como CPF/PGFN), mas com OUTRA frase ("não coincidem"). Divergência real do cadastro. | **Fix 2026-06-05 (#60):** `isDataDivergente` passou a cobrir "não coincide(m)" → `inconsistent_input` → **`data_invalid`** "Dados divergentes" + CTA "Corrigir". Sem isso, a MESMA divergência (ex.: cônjuge Dalice) ficava certa na Receita e errada no TRF5 (`failed_permanent` "use o portal"). |
